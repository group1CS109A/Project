...
The data augmentation helped to improve the test accuracy result to ~36%; it was a good improvement but still not as good as expected. In an attempt to further improve the accuracy results we used data augmentation and limed the images to show only the dog (removing  noise), cropping using bounding boxes as shown below, all these combined with the inclusion of the VGG19 in the network architecture.
